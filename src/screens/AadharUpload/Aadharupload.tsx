...
//       setAadharuploaddata((prev:any)=>({ ...prev, adharfileResponse: response }))
...
//   const [aadharsumbiteddata, setAadharsumbiteddata] = useState(params?.onboardingsumbiteddata); // Initialize with the submittedData
...
//           <TextInput
//             onChangeText={(text) => setAadharuploaddata({ ...aadharuploaddata, uploadAdharno: text })}
...
//             aadharuploaddata?.adharfileResponse ? <Image key={id.toString()} source={{ uri: file?.uri }} style={styles.img} /> : <Text>Choose File</Text>
//           ))}
...
export const Document = ({ navigation, route }) => {
  ...
  const [aadharsumbiteddata, setAadharsumbiteddata] = useState(params?.onboardingsumbiteddata);
  ...
  const handleAdharuploadSubmit = () => {
    ...
    if (!aadharuploaddata.uploadAdharno || aadharuploaddata.uploadAdharno.length !== 12) {
      setValidationErrors((prev) => ({ ...prev, aadharNumber: 'Please enter a valid Aadhaar card number (12 digits)' }));
      return;
    }
    ...
    <SafeAreaView style={styles.container}>
      <KeyboardAvoidingView behavior='padding'>
        <ScrollView>
          ...
          {aadharuploaddata.adharfileResponse?.map((file, id) => (
            <Image key={id.toString()} source={{ uri: file.uri }} style={styles.img} />
          ))}
